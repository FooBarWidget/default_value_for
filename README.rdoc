...
But when attributes with default values were set to nil e.g. through manual SQL the default values will be set on retrieval.
...
Explicitly set nil values will be overwritten by declared values, consider this example:

  user = User(:number => nil)
  user.number # => 123 # = declared default value
